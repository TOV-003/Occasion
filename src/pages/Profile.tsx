...
export default function ProfilePage() {
    const { profile, createdEvents, ownedCollectives, memberCollectives, attendingEvents } = useLoaderData() as {
        profile: ProfileType;
        createdEvents: Event[];
                ...
                <section className="rounded-2xl border border-inputaccent/20 bg-white p-5 shadow-sm md:p-6">
                    <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="inline-flex rounded-lg border border-inputaccent/30 bg-gray-100 p-1">
                            <button onClick={function () { setShowPast(false); }} className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${!showPast ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}>
                                Future
                            </button>
                            <button onClick={function () { setShowPast(true); }} className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${showPast ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}>
                                Past
                            </button>
                        </div>
                    </div>

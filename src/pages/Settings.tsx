            ...
            <div className="mx-auto max-w-3xl px-4 py-8 lg:px-8 lg:py-12">
                <div className="flex flex-col gap-6">
                    <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                        ...
                        {user && (<button type="button" onClick={function () {
                return navigate(`/profile/${user.id}`);
                    ...
                    <div className="flex items-center gap-4 rounded-2xl bg-inputbg/20 px-4 py-4">
                        <img src={profile?.avatar_url} alt="profile" className="h-20 w-20 rounded-full object-cover ring-4 ring-white/80"/>
                        <div>
                    ...
                    <div className="space-y-6 pb-2">
                        ...
                        <div>
                            <div className="mb-2 flex items-center justify-between gap-3">
                                ...
                                {!editingBio && (<button type="button" onClick={function () {
                return setEditingBio(true);
                            ...
                            {editingBio ? (<div className="space-y-3">
                                    <textarea value={bio} onChange={function (e) {
                return setBio(e.target.value);
            }} className="w-full rounded-xl border border-inputaccent/30 bg-white/70 p-3 text-sm text-gray-800 shadow-sm transition-colors focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20" rows={4} placeholder="Tell us about yourself..."/>
                                    <div className="flex flex-wrap gap-2">
                                        <button type="button" onClick={handleSaveBio} disabled={saving} className="rounded-lg bg-accent px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-accent-dark disabled:cursor-not-allowed disabled:opacity-60 cursor-pointer">
                ...
                setEditingBio(false);
                                ...
                                </div>) : (<div className="rounded-xl border border-inputaccent/20 bg-inputbg/20 p-3 text-sm leading-6 text-gray-700">
                    ...
                    <div className="flex flex-wrap gap-3 border-t border-inputaccent/20 pt-6">
                        <button type="button" onClick={handleLogout} className="rounded-lg bg-accent px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-accent-dark shadow-sm cursor-pointer">
                            Logout
                        </button>
                        <button type="button" onClick={handleDeleteAccount} className="rounded-lg bg-red-600 px-5 py-2.5 text-sm font-medium text-white transition-colors hover:bg-red-700 shadow-sm cursor-pointer">
                            Delete account
                        </button>
                    </div>
                </div>
